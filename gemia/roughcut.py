    ...
    *,
    ...
    if not clean_ids and all_assets:
        clean_ids = [
            ...
            for asset in list_assets(account_id, limit=max_assets, project_id=project_id)
    ...
    _rank_takes(account_id, ready, project_id=project_id)
...
def load_roughcut(account_id: str, asset_id: str, *, project_id: str | None = None) -> dict[str, Any]:
    if not get_asset(account_id, asset_id, project_id=project_id):
        raise RoughcutError("media asset not found")
...
def apply_roughcut_review(
    ...
    *,
    ...
    manifest = load_roughcut(account_id, asset_id, project_id=project_id)
    ...
    annotation = create_annotation(account_id, asset_id, annotation_payload, project_id=project_id)
...
def start_prepare_job(
    ...
    *,
    ...
    def _worker() -> None:
        ...
        try:
            result = prepare_roughcut(account_id, progress=_progress, project_id=project_id, **args)
...
def get_prepare_job(
    ...
    *,
    ...
    if str(record.get("project_id") or "") != str(project_id or ""):
        raise RoughcutError("rough-cut job not found")
...
def resume_prepare_job(
    account_id: str,
    job_id: str,
    *,
    project_id: str | None = None,
) -> dict[str, Any]:
    previous = get_prepare_job(account_id, job_id, project_id=project_id)
    payload = dict(previous.get("args") or {})
    payload["resume"] = True
    resumed = start_prepare_job(account_id, payload, project_id=project_id)
...
def _prepare_asset(
    ...
    *,
    ...
    _write_roughcut_annotations(account_id, manifest, project_id=project_id)
...
def _rank_takes(account_id: str, results: list[dict[str, Any]], *, project_id: str | None = None) -> None:
    groups: dict[str, list[dict[str, Any]]] = {}
    manifests: dict[str, dict[str, Any]] = {}
    for result in results:
        manifest = load_roughcut(account_id, str(result["asset_id"]), project_id=project_id)
...
def _write_roughcut_annotations(
    account_id: str,
