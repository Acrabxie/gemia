...
def build_segment_manifest(project: dict[str, Any]) -> list[dict[str, Any]]:
    """Describe every enabled Clip's expandable structure for delivery proof."""
    normalized = normalize_project(project)
    try:
        from gemia.segment_document import persisted_segment
        segments: list[dict[str, Any]] = []
        for clip in (normalized.get("timeline", {}).get("clips") or []):
            if not isinstance(clip, dict) or not bool(clip.get("enabled", True)):
                continue
            doc = persisted_segment(normalized, clip, create=False)
            segments.append({
                "clip_id": str(clip.get("id") or ""),
                "segment_id": str(doc.get("segment_id") or ""),
                "persisted": bool(doc.get("persisted")),
                "revision": int(doc.get("revision") or 0),
                "timeline": [{"id": str(item.get("id") or ""), "revision": int(item.get("revision") or 0), "start": float(item.get("start") or 0.0), "duration": float(item.get("duration") or 0.0)} for item in doc.get("timeline") or [] if isinstance(item, dict)],
                "layers": [{"id": str(item.get("id") or ""), "revision": int(item.get("revision") or 0), "visible": bool(item.get("visible", True))} for item in doc.get("layers") or [] if isinstance(item, dict)],
                "states": [{"id": str(item.get("id") or ""), "revision": int(item.get("revision") or 0), "dwell_sec": float(item.get("dwell_sec") or 0), "advance": str(item.get("advance") or "manual"), "visible_layer_ids": [str(layer_id) for layer_id in item.get("visible_layer_ids") or []]} for item in doc.get("states") or [] if isinstance(item, dict)],
            })
        return segments
    except Exception:
        return []
...
def build_graph_identity(project: dict[str, Any]) -> dict[str, Any]:
    ...
    segment_manifest = build_segment_manifest(normalized)
    segment_manifest_hash = _sha256_bytes(_canonical_json(segment_manifest))
...
def build_render_receipt(
    *,
    ...
    return {
        ...
        "segment_manifest": identity.get("segment_manifest", []),
        "segment_manifest_hash": identity.get("segment_manifest_hash"),
